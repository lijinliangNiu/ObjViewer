...
// 构建一个旋转矩阵，指定一个四元组旋转
void build_Rotmatrix(float m[4][4], float q[4]){//[x,y,z,w] //四元数组计算出旋转矩阵
...
// 追踪球函数返回一个gl矩阵
void trackBall_GL_Matrix(double m[16], float p1x, float p1y, float p2x, float p2y){
	float q[4];
